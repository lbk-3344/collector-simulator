...
## App icon / favicon — added 2026-09-02 (BL-077), mapping revised (BL-077a), topbar reverted (BL-077b)

**BL-077b (2026-09-02)**: the topbar goes back to the real **BarTender logo** (`public/brand/bartender-logo.png` + the "BarTender." wordmark, exactly as pre-BL-077). Luc's call — the original mark reads better in the header. The simulator mark stays on the favicon/PWA icon and, new here, becomes the **Devices** left-nav icon (monochrome, `currentColor`, so it flips white on the active row like the other nav icons). Login and pending-approval screens still show the simulator mark next to the wordmark (unchanged). So: BarTender logo = topbar only; simulator mark = favicon + Devices nav + auth screens + email.
...
- Wired into `app/layout.tsx` (`metadata.icons` — the SVG entry points at `icon-squircle.svg` since BL-077a, not `icon.svg`, so the vector favicon doesn't silently override the PNG/ICO white-on-orange fallbacks in browsers that prefer SVG icons; `metadata.manifest`; a `viewport.themeColor` of `#0D1E2C`). Used inline on `app/(auth)/login/page.tsx` and `app/(auth)/auth/pending/PendingPage.tsx` next to the "BarTender." wordmark, and as the **Devices** nav icon in `components/AppShell.tsx` (BL-077b — inline SVG, `viewBox="24 24 464 464"`, the same geometry as `icon.svg`). **The topbar reverted to `bartender-logo.png` at BL-077b** — the simulator mark is no longer in the header. At BL-077a, also wired into `lib/email.ts` (`emailMarkHtml()` helper, base64-inlined from `mark-32.png`) and used by `announcementEmailHtml()` there plus `startEmail()`/`resolvedEmail()` in `scripts/_bugs-lib.ts`.

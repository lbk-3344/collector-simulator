...
- [ ] **BL-032** — Per-user Bartender connection settings, decided 2026-08-27: new **Bartender Connection** tab in Settings, open to **every role** (not admin-gated, unlike Users) — each user enters their own Bartender tenant URL + API key. See `CLAUDE-CONCEPT.md` section 7.1. Needs: two new `User` fields (`bartenderTenantUrl`, `bartenderApiKeyCiphertext`) + a `bartenderApiKeyLast4` field for masked display, a Prisma migration, a new `ENCRYPTION_KEY` env var (AES-256-GCM, generate via `openssl rand -base64 32`, add to local `.env` + `.env.example` + both Vercel scopes), an encrypt/decrypt utility module, `GET`/`POST /api/settings/bartender` routes (POST never returns the plaintext key back), and the Settings UI form (masked key display, only re-encrypts on an actual new value). *(M)*
- [ ] **BL-033** — "Test connection" action + first live Bartender call, decided 2026-08-27, depends on BL-032: a button in the Bartender Connection tab that calls `GET {tenantUrl}/statemachine-api-configuration/rest/configuration/locations?level=premise` (header `apikey: <key>`) server-side — proxied through a new `POST /api/settings/bartender/test` route so the API key never reaches the browser's network tab — and reports success (with a location count) or a clear error. See `CLAUDE-CONCEPT.md` section 7.2 for the confirmed request; **response shape is not yet documented** — inspect the real response when first calling it with a valid key rather than assuming a shape, then come back and document it in section 7.2. *(S)*

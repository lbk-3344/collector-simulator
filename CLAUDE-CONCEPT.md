...
- **2026-08-30** — Workflows/Serialized Items design underway (resolving BL-003), still in discussion, nothing built yet. Confirmed so far: the reusable batch-of-items entity is named **Item Feed** (kind `NEW`/`PRESENT`/`FIXED`), and the per-Channel connection carrying items from one Task to the next is named **Flow Link** (superseding the "Item Source"/"Task Link" working names first drafted the same week). `FIXED` is confirmed to mean a literal, explicit list of EPCs — hex or URN form, user-entered — not a GTIN+quantity recipe. Luc explicitly wants the eventual build screen to be a node-and-edge graph editor matching the mockup's flow schematic (`mockups/item-sources-and-task-links.html`, Cowork-delivered, not yet in the repo) — Task nodes with one input per Channel, Flow Link edges carrying a delay range and an optional filter. Three new Bartender APIs documented the same day feeding this design: the legacy Serialization API (7.6, `NEW`), the legacy Product API (7.7, product/category picker, temporary until `master-data-api` is available), and the new Inventory API (7.8, `GET /stock`, `PRESENT`). Nothing formalized into a data model or `BACKLOG.md` yet — still following Luc's own step-by-step discussion pacing on this thread.

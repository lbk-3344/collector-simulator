...
#### Phase 0 live verification — done 2026-08-29 (BL-053)

Run against Luc's sandbox tenant `demotrackandtrace.sandbox.bartender-tt.com` (connection stored on the **staging** DB), key `…DIOD`. Throwaway collectors used for the checks were all deregistered afterward (`200 DEREGISTERED`); nothing test-only left on the platform.

1. **Sandbox gateway — confirmed.** `https://api.sandbox.bartender-tt.com/datacollector`, `apikey` header (same as `location-api-v2`). `GET /reference/read-point-types` → `200`, returns `{readPointTypes:[{code,label,category,icon,description}]}` — the 13-code set matches `READ_POINT_TYPES` exactly.
2. **API key is tenant-wide, NOT Location-scoped — confirmed, matches Luc's expectation.** `POST /collectors/register` with the one stored key succeeded at both `TTMEMBASE` and `TANDTWAREHOUSE` (`201` each). The `403 LOCATION_MISMATCH` risk is retired — Publish works at every site, no per-Location key model needed. (Tenant has ~33 locations; the three seeded sites are all present.)
3. **`POST /collectors/register` — observed contract.** First call → `201 {status:"REGISTERED"}`; re-register same `collectorId` → `200 {status:"UPDATED"}`. Idempotent on `collectorId`; the whole channel list is replaced each call (verified: re-registered `CH1` as `DIRECTIONAL` + added `CH2`, subsequent `GET` showed exactly those two). Response body: `{ collectorId, status, registeredAt, updatedAt, channelsAccepted, reconciliation }`.
   - **Accepted payload keys** (what this app sends): `collectorId`, `collectorName`, `locationId`, `model`, `vendor`, `readPointType`, `configVersion`, `attributes` (flat string/number/bool map), `channels: [{ channelId, channelType: "PRESENCE"|"DIRECTIONAL", presenceEvent (PRESENCE only), direction (DIRECTIONAL only), attributes? }]`.
4. **`reconciliation` — real shape (the spec's assumed shape was wrong).** `null` when nothing affected; otherwise `{ affectedMappings: [{ channelId, zoneId, zoneName, issue, detail, mappingStatus }] }`. `mappingStatus` ∈ `OFFLINE` / `CONFLICT` / `BROKEN` (spec; `BROKEN` not observed live); `issue` is a finer code (`COLLECTOR_OFFLINE`, `CHANNEL_TYPE_CHANGED`, …), `detail` is human text. **Because this app never sends heartbeats, every resync of an already-registered collector returns an `affectedMappings` entry with `mappingStatus: OFFLINE` / `issue: COLLECTOR_OFFLINE`** — expected and ignored; only `CONFLICT`/`BROKEN` entries surface per-Channel in the modal (15.4 / BL-054 frontend).
5. **`DELETE /collectors/{collectorId}` — observed contract.** `200 { mappingsRemoved, collectorId, status: "DEREGISTERED" }`; `404 { error: { code: "COLLECTOR_NOT_FOUND", field, message } }` when the id is unknown → **treated as success** for deregister-on-delete ("already gone" is fine).
6. **Error body shape.** `{ error: { code, field, message } }` (e.g. `{code:"VALIDATION_ERROR", field:"heartbeatConfig", message:"…"}`). `lastSyncError` is built from `error.message` (+ `error.code`), falling back to `HTTP {status}` — same convention as `POST /api/settings/bartender/test`.
7. **`heartbeatConfig` shape — UNRESOLVED, field omitted for now.** 10+ shapes for `heartbeatConfig` (object variants, boolean, number, JSON string) plus `heartbeat` and `heartbeatIntervalSeconds` all returned `400 VALIDATION_ERROR "… has a mismatched type."` — the validator knows the field names but the accepted type couldn't be brute-forced, and the `datacollector-api-v3` OpenAPI YAML isn't in the repo. **The register payload omits heartbeat config entirely**; a Device's `heartbeatEnabled` / `heartbeatTimeoutSeconds` stay local-simulator config only, not pushed to the platform. **TODO (needs Luc)**: supply the `heartbeatConfig` request schema from the API YAML so it can be mapped and included — same kind of live-API correction as BL-027/BL-041.

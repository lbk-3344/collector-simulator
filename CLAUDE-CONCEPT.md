...
- **`403 Forbidden` / `LOCATION_MISMATCH`** (not previously documented here) — "the submitted `locationId` does not match the Location scoped to the API key." Strong signal that a Bartender API key may be authorized for only one Location. Flagged as a real risk for this app's multi-site simulator in 15.8 — needs live verification against Luc's sandbox tenant before the real-publish feature ships.
...
**Auth & gateway** — reuses the same per-user Bartender Connection credentials as every other integration in this app (section 7.1): `bartenderTenantUrl` + the encrypted `apikey` (never Basic Auth for this API — no reason to take on the extra `x-tenant` header when the simple scheme already works for `location-api-v2`). Gateway resolution mirrors `lib/bartenderLocations.ts`'s `resolveGatewayUrl` pattern: sandbox when `tenantUrl` contains `sandbox`, production otherwise. **Flagged, unconfirmed**: the spec (section 7.5) documents only `https://api.bartender-tt.com/datacollector` (production) — no sandbox variant is written down, unlike `location-api-v2`, which turned out to need one (`api.sandbox.bartender-tt.com`) once tested live. `https://api.sandbox.bartender-tt.com/datacollector` is assumed by analogy and **must be live-verified before this ships**, not guessed further.
...
**Flagged risk — Location-scoped API keys**: the spec's own `403 LOCATION_MISMATCH` is defined as "the submitted `locationId` does not match the Location scoped to the API key" (section 7.5) — meaning a Bartender API key may only be authorized to register collectors at *one* Location. This app's stored API key is a single per-user credential, while the simulator has Devices seeded across three sites (`TTMEMBASE`, `TANDTWAREHOUSE`, `GRANITEFALLSSHOP`). **Must be live-verified against Luc's sandbox tenant before this feature is considered done** — attempt a real register call at two different `locationId`s with the same stored key and see whether both succeed or only one does. If the key really is single-Location-scoped, that's a real platform constraint to surface to Luc plainly (e.g. Publish only works at whichever Location the key is bound to, with a clear error elsewhere), not something to build around silently.

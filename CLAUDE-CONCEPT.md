...
- **Live-confirmed 2026-08-30** (one `quantity=1` mint against the sandbox tenant, `lib/bartenderSerialization.ts`): HTTP Basic auth with the stored `bartenderUsername`/`bartenderPassword` (`TTGlobalMgr`) works — same credential as 7.4. `gtin=03663328010013&quantity=1` → `200 ["3034DF978000FA400000005D"]` exactly as documented. **Wrinkle found**: the GTIN's GS1 check digit is validated — a GTIN copied straight from the Product API (7.7) can be *rejected* here (`00400499990061` → `400 {"code":400,"message":"The term 00400499990061 should have 4 instead of 1 as a check digit"}`). So a Product-API GTIN is not guaranteed to be valid Serialization-API input; the client surfaces the `400` message rather than pre-validating. Error body shape: `{ code, message }`.
...
- **Response shapes — live-confirmed 2026-08-30** (`lib/bartenderProducts.ts`, HTTP Basic `TTGlobalMgr`):
  - `GET /products?showAttributes=true` (`Accept-version: v3.6`) → `200`, a **plain JSON array** (not enveloped). Each entry: `{ gtin, displayGtin, productCode, productLabelShort, productLabelLong, categoryParent, categoryLevel1Code, categoryLevel1Label, sizeCode/sizeLabel, brandCode/brandLabel, modelCode/modelLabel, colorCode/colorLabel, seasonCode/seasonLabel, imageURL, prices: [{ value, currency, location }], extensions: {}, criticality, inScope, inScopeTraceability, attributes: [] }`. **197 products** on the sandbox today.
  - `GET /products/{gtin}` (`Accept-version: v3.5`) → `200`, the **same object minus `inScopeTraceability` and `attributes`** — the v3.5/v3.6 difference is real but minor (two fewer fields), not a shape change.
  - `GET /categories?show_level=true` (`Accept-version: v3.6`) → `200`, a **plain JSON array** of `{ categoryId, categoryName, categoryParent (""` for top level`), categoryLevel ("categoryLevel1"/"categoryLevel2"/…), creationDate/Time, updateDate/Time, updateUser, attributes?: [{ code, name, dataType, isRequired, validValues, defaultValue, extensionKey }] }`.
- **No server-side search or pagination** — `?page=`/`?pageSize=`/`?search=` on `/products` are all silently ignored (identical full array returned, no `x-total-count`/`link` headers). At 197 products this is fine: the picker fetches the full list once, caches it, and filters client-side by label/GTIN. **Flagged for Luc**: if the real catalog is much larger than the sandbox's 197, a server-side filter param (undocumented) or `master-data-api` will be needed before this scales.
...
- **Server — live-confirmed 2026-08-30**: `https://api.sandbox.bartender-tt.com/inventory` works (the `.sandbox.` counterpart does exist, same as 7.3/7.5); the production host `api.bartender-tt.com/inventory` returns `401` for the sandbox key. `resolveGatewayUrl` (`lib/bartenderLocations.ts`) reused, `/inventory` suffix.
- **Auth — live-confirmed 2026-08-30**: a **bare `apikey` header alone** authenticates (`200`) — the *same* scheme as 7.2/7.3/7.5, NOT the "Basic + apiKey together" the spec text implied. Sending HTTP Basic *instead of / alongside* makes the gateway demand an `x-tenant` header (`400 {"message":"x-tenant header is a must"}`) and then `401` with a plain tenant code — a dead end. So: `apikey` header only, no Basic, no `x-tenant`.
- **Key endpoint — `GET /stock`** — response shape **live-confirmed 2026-08-30**: `{ page, pageSize, total, asOf, inStockWindowDays, results: [...] }`. With `groupBy=sku` each result is `{ sku, locationCode, locationName, productLabel, qty }`; with `groupBy=zone` each also carries `{ zoneCode, zoneName, pid }` (PID = GTIN). No `lastSeenAt` seen in practice (spec listed it; treat as optional). `pageSize`/`page` **work** (`pageSize=5` honoured). **Filter params appear NOT to work server-side** in the sandbox: `locationCode=…`, `locationCode[]=…`, `zoneCode`, `pid` were all silently ignored (full unfiltered result set returned regardless). The sandbox only has ~6 stock rows total (all in `TANDTFACTORY`/`TANDTWAREHOUSE`), so `getStock` sends the documented filter params anyway **and also filters `results` client-side** by locationCode/zoneCode/pid as a safety net — revisit if the real API honours them. `groupBy` default is `sku`; the `PRESENT` Item Feed lookup uses `groupBy=zone`. An empty `results` (or `qty: 0`) is the "nothing to scan this run" case for `PRESENT`.
...
- **2026-08-30** — Workflow canvas + run engine Part 2 built (BL-060/061). `@xyflow/react` canvas at `/workflows/[id]` (list at `/workflows`); drag Devices → Tasks, drag handles → Flow Links, click channel/edge to configure. Server-side run engine (`lib/workflowRun.ts` + `GET /api/cron/workflow-tick`, `CRON_SECRET`-guarded): firing / in-flight arrivals / auto-stop, all idempotent-ish (claim-via-`updateMany`). **Phase 0 finding (live-checked)**: the ChefMate Vercel team is on **Hobby**, where Cron only fires once/day — so the tick is driven by an **external every-minute scheduler** (cron-job.org, `x-cron-secret` header), not native Vercel Cron. Setup steps in 16.5 / `.env.example`; swappable to `vercel.json` crons with no code change if the plan ever upgrades. `maxRunDurationMinutes` default **240 min confirmed** by Luc (editable per Workflow in the toolbar). New flags in 16.8: Flow Link category filters aren't evaluated yet (GTIN filters are), and `PRESENT` firings use placeholder item ids since `/stock` returns counts not EPCs.
- **2026-08-30** — Item Feeds + Tasks + Flow Links Part 1 built (BL-055–059). Three legacy API clients (`lib/bartenderProducts.ts`, `lib/bartenderSerialization.ts`, `lib/bartenderInventory.ts`) with all shapes live-verified against the sandbox — see 7.6/7.7/7.8. `ItemFeed` model + library UI (new "Item Feeds" nav item — Luc's pick over a Workflows tab); `Task`/`TaskChannelInput`/`FlowLink` models; `Workflow` gains automation fields (`maxRunDurationMinutes` default **240 min — placeholder, Luc to confirm**). **`Device.workflowId` removed**, superseded by `Device` → `Task` → `Workflow`; the Workflow dropdown is **gone from the Device config modal** (Luc's call — a Device joins a Workflow only by being dragged onto the Part 2 canvas as a Task). Three items flagged for Luc: (a) the legacy Product API has **no server-side search/pagination** — fine at ~197 sandbox products (fetch-all + client filter), needs a plan if the real catalog is large; (b) the Serialization API validates GS1 check digits, so not every Product-API GTIN is valid serialization input; (c) still-open from BL-053 — the DataCollector `heartbeatConfig` request shape (10+ shapes rejected, YAML not in repo). Category → GTIN resolution kept as "finder resolves to one GTIN at creation" per 16.1's working assumption. Part 2 (BL-060/061 — canvas + run engine) is a separate batch.
...
A named, reusable entity (create once, attach to any Task's Channel input across any Workflow — same "pick an existing thing" pattern as choosing a Device). Three kinds, picked at creation:

- **Common fields**: `name`; a product selector — either a specific GTIN or a category (searched via the legacy Product API, section 7.7, until `master-data-api` is available); `quantityMin`/`quantityMax` (equal values = a fixed count, otherwise randomized per firing) — quantity fields don't apply to `FIXED`.
- **`NEW`**: mints fresh EPCs via the legacy Serialization API (section 7.6) every time this Item Feed fires. **Hard-capped at 10 items per firing, app-enforced, regardless of the configured `quantityMax`** — Luc's explicit safety limit given each call is a real, permanent write to the live platform ("we get real serialized item from the platform... let's just allow not more than 10 new items at a time, we will level up the limit later when it is fully baked in"). No manual "generate now" escape hatch — it fires automatically like everything else, the cap is the safety net, not a human-in-the-loop step.
- **`PRESENT`**: also carries a `locationCode` + `zoneCode` (a specific site+zone, since Zones aren't globally unique) — at firing time, queries the Inventory API's `GET /stock` (section 7.8, `groupBy=zone`) for that GTIN/category in that zone and pulls up to the configured quantity from what's actually there. Empty or insufficient stock means fewer (or zero) items that firing — never an error, per Luc's original framing ("if none are present no items are scanned and send through the collector").
- **`FIXED`**: a literal, explicit list of specific items (EPC hex or URN, user-entered) — **confirmed 2026-08-30**, not a recipe. Fires the exact same items every time; quantity fields don't apply.

**Open runtime detail, not yet specified by Luc**: when the product selector is a category rather than a single GTIN, how a specific GTIN is picked at firing time (random pick within the category's products, one GTIN chosen at Item Feed creation via search, or all matching products included) isn't settled — default assumption for a first build: category search is a *finder* that still resolves to one concrete GTIN chosen at creation time, not a live per-firing random pick. Flag to Luc during/after implementation if that reading is wrong.

### 16.2 Task — a Device doing work in a Workflow

A Task attaches one Device to one Workflow (a Device belongs to at most one Task at a time — using it elsewhere means removing it from its current Task first). It gets **one input assignment per Channel** of that Device — not one shared batch for the whole Task — since Luc was explicit: "as many input as channels." Each Channel's input is exactly one of:

- an **Item Feed** (this Channel is an entry point — it fires on its own configured interval while the Workflow is RUNNING, minting/pulling/listing a fresh batch each time, per 16.5 below);
- an incoming **Flow Link** (this Channel receives whatever another Task's output sends it — see 16.3); a Channel can receive from any number of upstream Flow Links, no fan-in limit ("not limited, many upstream task to this one" — Luc's words);
- or **unused** (this Channel isn't fed by this Workflow at all).
...
### 16.3 Flow Link — a Channel-to-Channel connection

An edge from one Task's output Channel to another Task's input Channel (that input Channel's `TaskChannelInput` must be set to "Flow Link" — see 16.2). Carries:

- a **delay range** (`delayMinSeconds`/`delayMaxSeconds`) — how long an item takes to travel this hop, mirroring the quantity min/max randomization pattern already used elsewhere;
- an optional **filter** (GTIN list and/or category list) deciding which items take this edge;
- an **else** flag — at most one outgoing edge per Task/Channel can be the catch-all that receives anything no other edge's filter claims, so nothing is ever silently dropped when a Task branches (the dock → sort → storage-A/storage-B example).

### 16.4 Canvas UI — graph editor, not a form

Confirmed 2026-08-30 (mockups delivered: `item-feeds-and-flow-links.html`, `workflow-canvas-editor.html`, both Cowork-side, not yet in the repo). A left palette lists the user's configured Devices — dragging one onto the canvas creates a Task node showing one port per Channel (outbound-capable on the right, inbound on the left). Dragging from an output port to another Task's input port creates a Flow Link. Clicking an edge's label opens an inline panel to set its delay range and filter. Recommended library: `@xyflow/react` (React Flow, MIT) — its named-handle model maps directly onto Channels-as-ports, and it supports custom node/edge components for the port layout and the clickable delay/filter chip shown in the mockup. No live token animation for now (explicitly deferred by Luc) — a plain run/activity log is enough to see that automation is actually happening (see 16.5).
...
- **Scheduler mechanism**: Vercel Cron is the natural fit given this app's existing hosting, calling a protected API route on a fixed schedule to process due firings and check auto-stop deadlines. **Needs a live check before being relied on**: Vercel's cron-frequency limits differ by plan (a plan limited to daily invocations would make per-Task interval firing unworkable) — confirm what Luc's actual Vercel plan/tier allows before building around it; if native Cron can't fire often enough, fall back to an external free scheduler (e.g. cron-job.org) hitting the same protected route every minute via a shared secret, same idea as `CRON_SECRET`-style protection.
  - **Checked live 2026-08-30 (BL-061 Phase 0)**: the Vercel team hosting this project (**ChefMate**) is on the **Hobby** plan, where Cron jobs are capped at **one invocation per day** — nowhere near the ≤1-minute tick the per-Task `fireIntervalSeconds` design needs. So native Vercel Cron is **not** used. Instead: a `CRON_SECRET`-guarded route `GET /api/cron/workflow-tick` does all the work (firing / arrivals / auto-stop), and an **external every-minute scheduler** (e.g. cron-job.org, free tier) calls it. Setup Luc does once per environment: create a cron-job.org job on `* * * * *` → URL `https://<deployment-host>/api/cron/workflow-tick`, method `GET`, custom header `x-cron-secret: <value>`; set the same `CRON_SECRET` in Vercel env (Preview + Production) and local `.env`. The route rejects any request whose `x-cron-secret` (or `Authorization: Bearer <secret>`, Vercel's own convention) doesn't match. No `vercel.json` `crons` entry — deliberately, since Hobby would reject a sub-daily schedule anyway. If the project ever moves to Pro, a `vercel.json` `crons: [{ path: "/api/cron/workflow-tick", schedule: "* * * * *" }]` can replace the external trigger with no code change.

### 16.6 Data model implication — supersedes the placeholder Device↔Workflow link

Today's `Device.workflowId` (a direct FK, section 15.2's "minimal Workflow model... no creation/editing UI") is superseded by this design: a Device relates to a Workflow **through** a Task (`Device` 1:1 `Task`, `Task` many-to-one `Workflow`), not directly. `lib/deviceState.ts`'s Automated/Problem derivation moves from `device.workflow?.status` to `device.task?.workflow?.status` — same 4-state outcome, different path to it. Existing seed fixtures (2 Workflow fixtures, several Devices attached directly) get re-seeded as Tasks rather than migrated data-for-data, consistent with how earlier redesigns in this project (BL-042, BL-049) simply rewrote and re-ran the seed script rather than preserving fixture rows through a schema change.
...
- ~~Auto-stop default duration (16.5)~~ — **confirmed 2026-08-30**: Luc kept the 240 min (4 h) placeholder as the default, editable per Workflow in the canvas toolbar.
- **Flow Link category filters not evaluated yet** (BL-061) — a `FlowLink.filterGtins` list is matched literally against a batch's GTIN, but `filterCategoryCodes` is stored and ignored by the run engine (would need a GTIN→category lookup per batch, via the Product API). A category-only filter currently matches nothing; use a GTIN list or the `else` edge until this is wired.
- **`PRESENT` firings use placeholder item identifiers** (BL-061) — the Inventory API's `GET /stock` returns aggregate counts, not individual EPCs, so a `PRESENT` batch's `items` are synthetic strings (`present:{gtin}:…`) sized to `min(available, quantity)`. Fine for flow simulation; real EPC enumeration would need a different endpoint.
- **`heartbeatConfig` request shape** (carried over from BL-053) — still unresolved, still needs the `datacollector-api-v3` YAML from Luc.

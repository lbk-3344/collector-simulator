...
### 7.3 Location/Zone management API (`location-api-v2`) — provided 2026-08-27
...
- Auth: `X-API-Key` header or a Bearer token — **different from** the `apikey` header already confirmed working for the `statemachine-api-configuration` endpoint (section 7.2). Example server in the spec is a single fixed host, `https://api.bartender-tt.com/v1` — **not** obviously the same per-tenant subdomain pattern (`{tenantUrl}`) the already-built Bartender Connection settings (section 7.1) assume.

**Flagged, unresolved as of 2026-08-27**: this spec has not yet been live-tested against Luc's sandbox tenant the way section 7.2's endpoint was. Decided with Luc to build the new site-selector/map feature (section 14) against this API's shapes regardless, using the existing per-user `tenantUrl` + API key (section 7.1) as the best-guess connection — swap the request header to `X-API-Key` — but the actual reachable base URL and header need confirming against the real tenant (e.g. via the existing "Test connection" pattern, or a quick manual check) before treating the integration as done. If the fixed `api.bartender-tt.com` host in the spec turns out to be literal rather than a documentation placeholder, the per-user tenant-URL model itself may need revisiting for this specific API.

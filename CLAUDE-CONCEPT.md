...
### 7.3 Location/Zone management API (`location-api-v2`) — provided 2026-08-27, gateway architecture clarified same day
...
- `GET /locations` — paginated list of Locations, each with `code`, `name`, `type` (`DC`/`FACTORY`/`STORE`/`SUPPLIER`/`CUSTOMER`), `address`/`zipcode`/`city`/`state`/`country`, `lat`/`lng`, `hasMap` (boolean), `zoneCount`.
- `GET /locations/{code}/map` — floor-plan image metadata (`mapUrl`, `width`, `height`) for a Location, when `hasMap` is true. Not every Location has one — `SUPPLIER`/`CUSTOMER` type locations "may not have a map or zones" per the spec.
- `GET /locations/{code}/zones` — Zones within a Location, each with a `position` (`{x, y}` pixel coordinates on the floor-plan image) and a `type` driving default EPCIS semantics.
- Auth: `X-API-Key` header — **different from** the `apikey` header already confirmed working for the `statemachine-api-configuration` endpoint (section 7.2).

**Gateway architecture — clarified by Luc 2026-08-27, resolving the "different base URL" question flagged in the first version of this section**: unlike the `statemachine-api-configuration` endpoint (section 7.2), which is reached at the user's own per-tenant subdomain (`{tenantUrl}/...`), `location-api-v2` (and the platform's `DataCollector` API, not yet documented here) are reached through **one of two fixed API gateways**, not the tenant URL directly:

- **Production gateway**: `https://api.bartender-tt.com`
- **Staging gateway**: `https://api.staging.bartender-tt.com`

The per-user API key (already stored — section 7.1) is still what's sent, via the `X-API-Key` header — it's what the gateway uses to resolve which tenant the request is for. **Which of the two gateways to call is derived from the user's stored `bartenderTenantUrl`, not stored separately**: if `bartenderTenantUrl` contains `staging.bartender-tt.com` (e.g. `https://demotrackandtrace.sandbox.bartender-tt.com` does **not** match this — check the actual staging pattern against a real staging tenant URL when one's available, since the only tenant URL confirmed so far, used for section 7.2, is `demotrackandtrace.sandbox.bartender-tt.com`, whose staging/production status hasn't been explicitly stated by Luc), use the staging gateway; otherwise use the production gateway. Implement this as a small pure function (e.g. `resolveGatewayUrl(tenantUrl): string`) rather than hardcoding the choice inline, since the exact substring match may need adjusting once tested against a real staging tenant.

**Live-tested 2026-08-27 against the sandbox tenant's key (`XCONEBOUVGDPDIOD`, confirmed working for section 7.2) — does not work as specified. Flagged for Luc, not wired up as a working integration:**

- **Staging gateway unreachable**: `curl -v https://api.staging.bartender-tt.com/locations` — DNS resolves, TCP connects, TLS handshake begins and stalls after the server sends its certificate (`CERT verify` step) — no response ever comes back, request just hangs until timeout. Looks like an infrastructure/cert issue on that host, not something fixable from this app's side.
- **Production gateway rejects the sandbox key**: `curl https://api.bartender-tt.com/locations` —
  - With the header as spec'd, `X-API-Key: XCONEBOUVGDPDIOD` → `401`, JSON body `{"message":"apikey or basic auth is required"}`. Reads like the gateway doesn't recognize `X-API-Key` as a supported header name at all (message implies *no* auth was found).
  - With `apikey: XCONEBOUVGDPDIOD` (lowercase, matching section 7.2's header instead) → `401`, plain-text body `Unauthorized` — a *different* error, which suggests this header name **is** being read (got past the "is anything present" check to an actual key-validation step), but the sandbox key isn't valid for this gateway.
  - Net effect: neither header name authenticates successfully with the one key available. Possibilities, unconfirmed: the header name really is `apikey` (not `X-API-Key`) but gateway-level auth needs a different credential than the tenant's Track & Trace API key; or this sandbox tenant/key just isn't provisioned for `location-api-v2`/the gateway product yet; or something else entirely.

**Implication for BL-036–039**: the `resolveGatewayUrl`/`listLocations`/`getLocationMap`/`getLocationZones` client and the API routes that call it are built exactly to this spec (see `lib/bartenderLocations.ts`), but calls through them will currently fail — the UI (Site selector, Location map cards) is built to degrade gracefully (a clear error state, not a crash or silent fake data) rather than pretend this is working end-to-end. Needs Luc to confirm the actual header name and/or credential for the gateway, and to unblock or explain the staging host, before this can be verified live.
